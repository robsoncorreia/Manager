...
{
    public class MainViewModel : ProjectViewModel
    {
        ...
        public string Title { get; } = $"{Properties.Resources.AppName} {Properties.Resources.AppVersion}.{ENV}";

        private void Loaded()
        {
            if (ParseUser.CurrentUser != null)
            {
                _navigationService.NavigateTo(Domain.Properties.Resources.DashBoard);
            }
            else
            {
                _navigationService.NavigateTo(Domain._Util.AppConstants.LOGIN);
            }
        }
    }
}