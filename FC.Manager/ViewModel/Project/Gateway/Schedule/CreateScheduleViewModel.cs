...
{
    public class CreateScheduleViewModel : ProjectViewModelBase, IDropTarget
    {
        ...
        private bool _IsOpenDialogHostCalendar;
        private ZwaveDevice _ScheduleModel;

        public CreateScheduleViewModel(IFrameNavigationService navigationService,
                                 ...
                                 IIfThenRepository ifThenRepository) : base(navigationService, projectService, udpRepository, tcpRepository, irRepository, userService, projectRepository, localDBRepository, logRepository, userRepository, serialRepository, commandRepository, ipCommandRepository, gatewayService, configurationRepository, taskService, parseService, rfRepository, zwaveRepository, relayTestRepository, gatewayRepository, dialogService, loginRepository)
        {
            ...
            CloseDrawerCommand = new RelayCommand<object>(CloseDrawer);

            GetEndpointValueCommand = new RelayCommand<object>(GetEndpointValue);

            EnabledCommand = new RelayCommand<object>(Enabled);

            OpenDialogHostCalendarCommand = new RelayCommand<object>(OpenDialogHostCalendar);

            ChangeTabCommand = new RelayCommand<object>(ChangeTab);

            SelectedAllDaysCommand = new RelayCommand<object>(SelectedAllDays);

            NewCommand = new RelayCommand<object>(NewAsync);

            UnloadedCommand = new RelayCommand<object>(Unloaded);

            SelectedIfThenModel = new IfThenModel { IfthenType = IfthenType.Schedule };
        ...
        public ZwaveDevice ScheduleModel
        {
            get => _ScheduleModel;
            set => SetProperty(ref _ScheduleModel, value);
        }

        public bool IsOpenDialogHostCalendar
        {
            get => _IsOpenDialogHostCalendar;
            set => SetProperty(ref _IsOpenDialogHostCalendar, value);
        }
        ...
        {
            if (SelectedIfThenModel.ZwaveDevicesThen.Count == 0)
            {
                OpenDrawer();
                throw new ScheduleException(Domain.Properties.Resources.At_least_one_action_is_required_);
            }

            long minValueSeconds = 30;
            long minValueMinutes = 1;
            long minValueHours = 1;
            long minValueDays = 1;
            long minValueMonths = 1;
            long minValueYears = 1;

            switch (ScheduleModel.SelectedIndexTabSchedule)
            {
                case 0:

                    switch ((DateTypeEnum)ScheduleModel.SelectedIndexDateType)
                    {
                        case DateTypeEnum.EveryAmountSeconds:
                            if (ScheduleModel.ValueSchedule < minValueSeconds)
                            {
                                throw new ScheduleException(string.Format(Domain.Properties.Resources.Time_must_be_greater_than_or_equal_to_x_, minValueSeconds, minValueSeconds > 1 ? Domain.Properties.Resources.Seconds.ToLower() : Domain.Properties.Resources.Second.ToLower()));
                            }
                            break;

                        case DateTypeEnum.EveryAmountMinutes:
                            if (ScheduleModel.ValueSchedule < minValueMinutes)
                            {
                                throw new ScheduleException(string.Format(Domain.Properties.Resources.Time_must_be_greater_than_or_equal_to_x_, minValueMinutes, minValueMinutes > 1 ? Domain.Properties.Resources.Minutes.ToLower() : Domain.Properties.Resources.Minute.ToLower()));
                            }
                            break;

                        case DateTypeEnum.EveryAmountHours:
                            if (ScheduleModel.ValueSchedule < minValueHours)
                            {
                                throw new ScheduleException(string.Format(Domain.Properties.Resources.Time_must_be_greater_than_or_equal_to_x_, minValueHours, minValueHours > 1 ? Domain.Properties.Resources.Hours.ToLower() : Domain.Properties.Resources.Hour.ToLower()));
                            }
                            break;

                        case DateTypeEnum.EveryAmountDays:
                            if (ScheduleModel.ValueSchedule < minValueDays)
                            {
                                throw new ScheduleException(string.Format(Domain.Properties.Resources.Time_must_be_greater_than_or_equal_to_x_, minValueDays, minValueDays > 1 ? Domain.Properties.Resources.Days.ToLower() : Domain.Properties.Resources.Day.ToLower()));
                            }
                            break;

                        case DateTypeEnum.EveryAmountMonths:
                            if (ScheduleModel.ValueSchedule < minValueMonths)
                            {
                                throw new ScheduleException(string.Format(Domain.Properties.Resources.Time_must_be_greater_than_or_equal_to_x_, minValueMonths, minValueMonths > 1 ? Domain.Properties.Resources.Months.ToLower() : Domain.Properties.Resources.Month.ToLower()));
                            }
                            break;

                        case DateTypeEnum.EveryAmountYears:
                            if (ScheduleModel.ValueSchedule < minValueYears)
                            {
                                throw new ScheduleException(string.Format(Domain.Properties.Resources.Time_must_be_greater_than_or_equal_to_x_, minValueYears, minValueYears > 1 ? Domain.Properties.Resources.Years.ToLower() : Domain.Properties.Resources.Year));
                            }
                            break;
                    }
                ...
                case 1:
                    if (ScheduleModel.DaysOfWeekList.FirstOrDefault(x => x.IsSelected == true) is null)
                    {
                        throw new ScheduleException(Domain.Properties.Resources.You_must_select_at_least_one_day_of_the_week_);
                    }
                    break;

                default:
                    throw new ScheduleException(Domain.Properties.Resources.Function_not_implemented);
            }
            return true;
        }
            ...
            try
            {
                ...
                OpenCustomMessageBox(header: Domain.Properties.Resources.Delete,
                                     message: string.Format(CultureInfo.InvariantCulture,
                                              string.Format(Domain.Properties.Resources.Do_you_want_to_delete____,
                                              Domain.Properties.Resources.Scheduling,
                                              SelectedIfThenModel.Name), SelectedIfThenModel.Name),
                                     custom: () => Delete(),
        ...
        private async void GetIfThen(IfThenModel selectedIfThenModel)
        {
            try
            {
                ...
                if (selectedIfThenModel.ParseObject != null)
                {
                    ...
                    ScheduleModel = SelectedIfThenModel.ZwaveDevicesIf.FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
        ...
        private void OpenDrawer(object obj = null)
        {
        ...
        private async Task<bool> Create(object obj = null)
        {
            bool @return = false;
            ...
            try
            {
                ...
                OpenCustomMessageBox(header: Domain.Properties.Resources.Wait,
                                     isProgressBar: true,
                                     message: Domain.Properties.Resources.Creating_schedule);

                SelectedIfThenModel.ZwaveDevicesIf.Clear();

                if (SelectedIfThenModel.ParseObject is null)
                {
                    ScheduleModel.ParseObject = null;
                }

                SelectedIfThenModel.ZwaveDevicesIf.Add(ScheduleModel);

                await _ifThenRepository.CreateSchedule(SelectedProjectModel, SelectedIfThenModel);

                await _ifThenRepository.Update(SelectedProjectModel, SelectedIfThenModel);

                await CloseDialog();

                action = () => { IsActiveSnackbar = false; };

                IsActiveSnackbar = true;

                ContentSnackbar = Domain.Properties.Resources.It_is_necessary_to_restart_the_gateway__Restart_now_;
            ...
            catch (Exception ex)
            {
                ...
                OpenCustomMessageBox(header: Domain.Properties.Resources.Error,
                                     message: $"{ex.Message}\n\n{Domain.Properties.Resources.Try_again}?",
        ...
        private void NewAsync(object obj)
        {
            if (SelectedIfThenModel.ParseObject is null)
            {
                OpenCustomMessageBox(header: Domain.Properties.Resources.New,
                                     message: Domain.Properties.Resources.The_schedule_has_not_been_saved__Want_to_save_before_creating_a_new_one_,
                                     cancel: async () => await CloseDialog(),
        ...
        private void SelectedAllDays(object obj)
        {
            if (obj is not bool isChecked)
            {
                return;
            }
            foreach (DaysOfWeekModel day in ScheduleModel.DaysOfWeekList)
            {
                day.IsSelected = isChecked;
            }
        }

        private void OpenDialogHostCalendar(object obj)
        {
            IsOpenDialogHostCalendar = !IsOpenDialogHostCalendar;
        }

        private void CloseDrawer(object obj)
        {
            IsRightDrawerOpen = false;
            IsExpandedIf = false;
            IsExpandedElse = false;
            IsExpandedThen = false;
        }
        ...
        public ICommand SelectedAllDaysCommand { get; set; }
        ...
        public ICommand OpenDialogHostCalendarCommand { get; set; }
